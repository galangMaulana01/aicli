...
def run_cli():
    ...
    while True:
        ...
        # Gunakan while loop untuk menangkap return value dari generator
        while True:
            try:
                chunk = next(gen)
                if not isinstance(chunk, str):
                    continue
                # Jangan tampilkan tool result mentah
                if "[TOOL_RESULT]" in chunk:
                    continue
                # Jangan tampilkan pesan stop
                if chunk == "[STOP] max iteration reached":
                    continue
                print(chunk)
            except StopIteration as e:
                # Tangkap return value = history yang sudah di-update
                history = e.value
                break
        print()
